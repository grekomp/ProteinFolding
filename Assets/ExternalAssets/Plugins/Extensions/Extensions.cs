...
public static class ExtensionMethods {
	...
	public static void LoadSpriteFromDisc(this MonoBehaviour mb, string path, System.Action<Sprite> callback) {
		mb.StartCoroutine(LoadFromDiscCoroutine(path, www => {
			Sprite sprite;
			if (string.IsNullOrEmpty(www.error)) {
				sprite = Sprite.Create(www.texture, new Rect(0, 0, www.texture.width, www.texture.height), Vector2.one / 2f);
			}
			else {
				Debug.LogError("Loading error: " + www.error);
				sprite = Resources.Load<Sprite>("Textures/PlaceholderCover");
			}
			if (callback != null) {
				callback(sprite);
			}
	...
	public static void LoadAudioFromDisc(this MonoBehaviour mb, string path, System.Action<AudioClip> callback) {
		mb.StartCoroutine(LoadFromDiscCoroutine(path, www => {
			if (string.IsNullOrEmpty(www.error)) {
				if (callback != null) {
					AudioClip ac = www.GetAudioClip(false, false, AudioType.UNKNOWN);
					callback(ac);
				}
			}
			else {
	...
	static System.Collections.IEnumerator LoadFromDiscCoroutine(string path, System.Action<WWW> callback) {
